...
namespace PerfLint.Scanners
{
    ...
    ///            (Scene-level counterparts, both scene-grounded & no-op instancing: MAT004 / InstancingBatchingScanner — instancing overlapping
    ///            Static Batching, preempted; MAT005 / SkinnedInstancingScanner — instancing on skinned-only materials, where it never applies.)
    ///   MAT003 — Material's shader reference is missing/None (deleted asset, uninstalled package) → renders magenta.
    /// </summary>
    public sealed class MaterialScanner : IScanner, IFileScanner
    {
        public string Name => "Materials";
        public Domain Domain => Domain.Performance;

        // GPU/native reclaim throttle counter (see ScannerUtil.ThrottleReclaim). Instance field so the full Scan loop's
        // loop-top reclaim and single-file ScanFile share it; reset per Scan. Main-thread only.
        private int _loadsSinceReclaim;

        public IEnumerable<Finding> Scan(ScanContext context)
        {
            ResolvePipeline(out bool isSrp, out bool isUrp);
            ...
            for (int i = 0; i < guids.Length; i++)
            {
                ...
                // Loading a material pulls in the textures it references and uploads them to the GPU. Across the non-yielding
                // scan loop those accumulate until the driver OOM-crashes on large projects. Reclaim prior iterations' loads at
                // loop top — NOT mid-iteration: the sweep could unload `mat` itself while the branches below are still reading it.
                _loadsSinceReclaim = ScannerUtil.ThrottleReclaim(_loadsSinceReclaim);

                string path = AssetDatabase.GUIDToAssetPath(guids[i]);
                foreach (var f in ScanMaterial(path, isSrp, isUrp)) yield return f;
            }
        }

        /// <summary>Path-based (extension) gate for incremental re-scan — see the note on TextureImportScanner.Handles. ScanFile does the precise load/null check.</summary>
        public bool Handles(string assetPath)
            => !string.IsNullOrEmpty(assetPath) && Path.GetExtension(assetPath).ToLowerInvariant() == ".mat";

        /// <summary>Incremental single-file scan: the same MAT00x rules as the full Scan, for one material. Empty when the asset can't be loaded as a Material.</summary>
        public IEnumerable<Finding> ScanFile(string assetPath, ScanContext context)
        {
            if (!Handles(assetPath)) return System.Array.Empty<Finding>();
            ResolvePipeline(out bool isSrp, out bool isUrp);
            _loadsSinceReclaim = 0;
            return ScanMaterial(assetPath, isSrp, isUrp);
        }

        /// <summary>Resolves the active render pipeline once: isSrp (URP/HDRP vs Built-in) and isUrp (URP specifically, for the one-click upgrade Action).</summary>
        private static void ResolvePipeline(out bool isSrp, out bool isUrp)
        {
            var rp = GraphicsSettings.currentRenderPipeline; // null = Built-in pipeline
            isSrp = rp != null;
            // Whether the active SRP is URP (as opposed to HDRP) — the "one-click upgrade to URP" Action is only exposed for
            // URP. We do not reference the URP package directly; instead we identify it by the render pipeline asset's type
            // name (URP uses UniversalRenderPipelineAsset). HDRP's upgrader is different and higher risk — out of scope.
            isUrp = isSrp && rp.GetType().FullName?.Contains("Universal") == true;
        }

        /// <summary>
        /// All MAT00x rules for a single material, in the original order (MAT003 short-circuits, else MAT001 then MAT002).
        /// Shared by the full Scan loop and single-file ScanFile so the two are identical per asset (asserted by
        /// ScanFile_MatchesFullScan_PerMaterial). Loads the material; the full Scan loop reclaims at its loop top.
        /// </summary>
        private List<Finding> ScanMaterial(string path, bool isSrp, bool isUrp)
        {
            var findings = new List<Finding>();

            var mat = AssetDatabase.LoadAssetAtPath<Material>(path);
            if (mat == null) return findings;
